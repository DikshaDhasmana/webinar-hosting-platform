...
export default function AdminWebinarRoom({ params }: { params: Promise<{ id: string }> }) {
  ...
  const initializeWebinar = async () => {
    try {
      ...
      socket.on('webinar-joined', (data) => {
        console.log('Joined webinar as admin:', data);
  ...
  const startWebinar = () => {
    socketRef.current?.emit('start-webinar');
  };

  const endWebinar = () => {
    socketRef.current?.emit('end-webinar');
  };
      ...
      <div className="bg-gray-800 p-4">
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-400">Host: {webinar?.hostName} (Admin)</p>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 h-full">
            ...
            <div className="bg-gray-800 rounded-lg overflow-hidden">
              <div className="p-2 bg-gray-700">
                <h3 className="text-sm font-medium">You (Host)</h3>
              </div>
              <video
                ref={localVideoRef}
                autoPlay
